...
export const TeacherItem = ({ name, description, age, experience, id, img, className }) => {
    ...
    return (
        <>
            {
                renderTeacher &&
                <div className="fixed size-full bg-gray-900/50 backdrop-blur-md top-0 right-0 z-20 flex items-center text-gray-100 text-2xl">
                    <div className=" ml-16 flex flex-col text-xl font-normal w-1/2">
                        <h2 className="font-extrabold mb-6 text-7xl text-yellow-400">{name}</h2>
                    ...
                    <img className="w-[48%] absolute right-0" src={img} alt={`Coach ${name}`} />
                </div>
            }
